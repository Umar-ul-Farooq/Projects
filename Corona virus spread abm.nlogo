...
if color != blue [
    if social_distancing [
      let turtle_in_front one-of turtles-on patch-ahead 4.1
  ...
  if color = green and infected_time <= 0 and not self_isolation [
...
if self_isolation and color = green and infected_time < (illness_duration - undetected_period) [
    set color blue

  ]

if self_isolation and color = blue and infected_time <= 0 [
      ifelse random 100 <= survival_rate [
        set color black
        set infected_time 0
      ] [
...
to model_outputs
  ...
  let surviving_population current_pink_population + current_gray_population
  ...
  if surviving_population > 0 or current_pink_population > 0 or current_gray_population > 0 [
  ...
  let total_with_antibodies count turtles with [color = black]
  set total_antibodies_percentage (total_with_antibodies / surviving_population) * 100

  let pink_with_antibodies count turtles with [color = black and group = "pink turtle"]
  set pink_antibodies_percentage (pink_with_antibodies / current_pink_population) * 100

  let gray_with_antibodies count turtles with [color = black and group = "gray turtle"]
